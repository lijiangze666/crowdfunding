...
const PopUp = ({setOpenModel, donate, donateFunction, getDonations}) => {
    ...
    useEffect(() => {
        ...
        return async () => {
            const allData = await donationListData
            console.log('allData',allData)
            setAllDonationData(allData)
        }
                        ...
                        <div
                            ...
                            <div className="relative p-6 flex-auto">
                                ...
                                <input
                                    ...
                                    placeholder="amount"
                                    required
                                    className="flex-grow w-full px-4 py-2 mt-2 transition duration-200 bg-white border border-gray-300
                                    rounded shadow-sm appearance-none foucus:border-deep-purple-accent-400 focus:outline-none focus:shadow-outline"
                                    id="amount"
                                    name="amount"
                                />
                                {allDonationData.map((data, i) => (
                                    <p
                                        key={i+1}
                                        className="my-4 text-slate-500 text-lg leading-relaxed">
                                        {i + 1}:{data.donation}/
                                        {`${data.donator.slice(0, 5)}...${data.donator.slice(data.donator.length - 4)}`}
                                    </p>
                                ))}
                            </div>
                            {/*  footer  */}
                            ...
                            rounded-b">
                                ...
                                <button
                                    className="background text-white active:bg-emerald-600 font-bold uppercase text-sm
